...
void dht_calculate_hash_test()
{
   ...
   uint160_t *hash1;
   uint160_t *hash2;
   dht_calculate_hash((const uint8_t *)data1, hash1);
   dht_calculate_hash((const uint8_t *)data2, hash2);
...
void dht_insert_test(struct DHT *dht)
{
   printf("DHT INIT\n");
   struct Node *node = get_test_node("127.0.0.1", 8080);
   dht_insert(dht, node);
   printf("Buckets: %d Nodes: %d\n", dht->num_buckets, dht->num_nodes);
   // CU_ASSERT_EQUAL(dht->buckets[0]->nodes[0][0].port, 8080);
   // printf("PORT: %s", dht->buckets[0]->nodes[0][0].host);
   dht_free_node(node);
}
